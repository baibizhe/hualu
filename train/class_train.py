...
def train_class():
    ...
    all_imgs = [''.join([os.path.join(train_imgs, str(i) + '.png')]) for i in names][1:500] # ONLY CHOOSE 200 SAMPLES TO PROVE THE DATA DOPPELGÄNGERS
    # masks = [''.join([os.path.join(train_masks, str(i) + '.png')]) for i in names][1:100]
    ...
    for i in range(5):
        ...
        mb_encoder_path = '/home/project/temp_data/mobilenet_v3_large-8738ca79.pth'

        model.load_state_dict(torch.load(mb_encoder_path))
        ...
        train_labels = labels[:i * c] + labels[(i + 1) * c:]
        test_imgs = imgs[i * c:(i + 1) * c]
        test_labels = labels[i * c:(i + 1) * c]
    ...
    print('CV_Acc', np.mean(best_acc_final))
